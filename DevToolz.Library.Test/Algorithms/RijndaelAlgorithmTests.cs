...
public class RijndaelAlgorithmTests
{
    private ICryptAlgorithm Algorithm => CryptFactory.Create( CryptProvider.Rijndael );
    ...
    public void Encrypt_ThenDecrypt_ShouldReturnOriginalValue()
    {
        ...
        const string original = "Hello, Rijndael!";
    ...
    // Cenários de diagnóstico: causas comuns do erro "Padding is invalid" em produção

    [Fact]
    public void Decrypt_WhenKeyHasLeadingSpace_ShouldThrowCryptographicException()
    {
        // Diagnóstico: chave com espaço extra (ex: config lida sem Trim())
        var algorithm    = Algorithm;
        string encrypted = algorithm.Encrypt( "dado", "minhaChave" );

        Assert.Throws<CryptographicException>( () => algorithm.Decrypt( encrypted, " minhaChave" ) );
    }

    [Fact]
    public void Decrypt_WhenKeyHasTrailingSpace_ShouldThrowCryptographicException()
    {
        // Diagnóstico: chave com espaço trailing (ex: campo de texto sem Trim())
        var algorithm    = Algorithm;
        string encrypted = algorithm.Encrypt( "dado", "minhaChave" );
    ...
    [Fact]
    public void Decrypt_WhenKeyHasDifferentCase_ShouldThrowCryptographicException()
    {
        // Diagnóstico: chave com case diferente (ex: normalização inconsistente)
        var algorithm    = Algorithm;
        string encrypted = algorithm.Encrypt( "dado", "MinhaChave" );
    ...
    [Fact]
    public void Decrypt_WhenCiphertextIsTruncated_ShouldThrowCryptographicException()
    {
        // Diagnóstico: ciphertext truncado (ex: coluna de banco com tamanho insuficiente)
        var algorithm    = Algorithm;
        string encrypted = algorithm.Encrypt( "dado para truncar", "chave" );
        string truncated = encrypted[ ..( encrypted.Length / 2 ) ];

        Assert.ThrowsAny<Exception>( () => algorithm.Decrypt( truncated, "chave" ) );
    }

    [Fact]
    public void Decrypt_WhenCiphertextHasExtraCharacters_ShouldThrowCryptographicException()
    {
        // Diagnóstico: ciphertext com bytes extras (ex: concatenação errada ao persistir)
        var algorithm    = Algorithm;
        string encrypted = algorithm.Encrypt( "dado", "chave" );
        string corrupted = encrypted + "AA==";

        Assert.ThrowsAny<Exception>( () => algorithm.Decrypt( corrupted, "chave" ) );
    }
}
